...
const users_id_collections_post = async (request, response) => {
    ...
    console.log(collectionToAdd)

    try {
        const collectionInDB = await Collection.create(collectionToAdd)
        await response.send(collectionInDB)
    } catch ({errors}) {
        const collectionNameFailed = !collectionToAdd.name_collection || collectionToAdd.name_collection>collectionNameCharLength ? collectionNameCharLengthFailedText : emptyString
